...
def get_order_by_user(id):
    with closing(connection.cursor()) as cursor:
        cursor.execute(""" SELECT food_order.id, food_customer.first_name,food_customer.last_name,food_customer.email,food_customer.country, food_order.address, food_order.payment_type,food_order.status,food_order.created_at from food_order 
                            INNER JOIN food_customer 
                            on food_customer.id=food_order.customer_id 
...
def get_product_by_order(id):
    with closing(connection.cursor()) as cursor:
        ...
        ON food_orderproduct.product_id=food_product.id  
        where order_id=%s""",[id])
...
INNER JOIN food_product 
ON food_product.id=food_orderproduct.product_id 
